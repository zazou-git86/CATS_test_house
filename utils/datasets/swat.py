...
class Swat(Dataset):
    ...
    def __init__(self, seed: int, shorten_long=True, remove_unique=False, entity=None, verbose=False, one_hot=False):
        ...
        root = os.path.join("datasets", "swat")
    ...
    def load(self):
        ...
        OUTLIER_CLASS = 1
        test_df: pd.DataFrame = pd.read_csv(self.raw_path_test, low_memory=False)
        train_df: pd.DataFrame = pd.read_csv(self.raw_path_train, low_memory=False)
