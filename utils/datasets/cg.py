...
from utils.datasets.window import WindowAnomaly
...
def identify_window_anomaly(window, window_size, threshold, pos_val=1.0, is_consecutive=False):
    ...
    if total_anomal_obs >= threshold:  #(v >= threshold): # or
            #an.append(1)
        return 1
    # elif total_anomal_obs == 0:
    #     return 0
    else:
        return 0


def window_processing(cg_df: pd.DataFrame, window_size=10,
                        threshold=10, stride=10, is_consecutive=False):
    ...
    Returns:
        dict: Data dicts.
    """
    cg_win = get_sub_seqs(cg_df.values, window_size, stride=stride)
    x_list = []
    y_list = []
...
def get_all_data(df_dicts, window_size=10, threshold=10, stride=10, is_consecutive=False):
...
def train_test_split(normal_arr, anomaly_arr, seed=42):
    ...
    np.random.seed(seed)
    shuffled_indices = np.random.permutation(len(normal_arr))
    test_ratio = 0.5
    ...
    shuffled_indices = np.random.permutation(len(anomaly_arr))
...
def split_contamination_data(train_arr, contamination_ratio, window_size=10, seed=42,
    ...
    if contamination_ratio==0:
        ...
        return {'x' : train_x[:,:, :14], 'window_labels': train_win_y, 'y': train_y}
    elif contamination_ratio==1:
        print("Kept all the anomalous windows in the training set for contamination.")
        return {'x' : train_arr[:,:, :14], 'window_labels': window_labels, 'y': y_arr}

    elif contamination_ratio < 1:
        np.random.seed(seed)
        shuffled_indices = np.random.permutation(len(anomaly_index_list))
...
def get_x_y_data(all_data, window_size=10, threshold=10, wad_threshold=10, is_consecutive=False):
    ...
    return {'x' : all_data[:,:, :14], 'window_labels': window_labels, 'y': y_arr,
