...
from utils.datasets.dataset import get_events
...
class Wadi(Dataset):
    ...
    def __init__(self, seed: int, remove_unique=False, entity=None, verbose=False, one_hot=False):
        ...
        self.raw_path_train = os.path.join("datasets", "wadi", "old",  "WADI_14days.csv")
        self.raw_path_test = os.path.join("datasets", "wadi", "old", "WADI_attackdata.csv")
        self.anomalies_path = os.path.join("datasets", "wadi", "old", "table_WADI.csv")
        self.seed = seed
    ...
    def load(self):
        ...
        ano_df = pd.read_csv(self.anomalies_path, header=0, sep=',')
        ...
        for i in range(ano_df.shape[0]):
            ...
            test_df.loc[start_row:end_row, 'y'] = np.ones((1 + end_row - start_row,1))

            causes.append(ano_df.iloc[i, :]["Attack Identifier"])
