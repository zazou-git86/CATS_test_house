...
class SimCLR(nn.Module):
    """SimCLR implementation for Anomaly Detection.
    """
...
def train(train_loader, model, loss_cons_fn, optimizer, device, scheduler):
    ...
    for batch in train_loader:
        ...
        _, pos_embedding_1 = model(pos_1)
        _, pos_embedding_2 = model(pos_2)

        loss = loss_cons_fn(pos_embedding_1, pos_embedding_2, None, cluster=False)
...
def fit_with_early_stopping(train_loader, val_loader, loss_cons_fn,
...
def eval_on_data(dataset, save_dir, output_size, proj_size,
                ...
                use_gpu=True):
    """Train and evaluate SimCLR on a dataset.
    ...
    model = SimCLR(input_size=feat_size,
    ...
    model = fit_with_early_stopping(train_loader, val_loader, loss_cons_fn,
                            model, patience, num_epochs, learning_rate,
                            writer, device, weight_decay=1e-5,
                            verbose=False)
...
def load_model(save_dir, device):
    ...
    model = SimCLR(input_size=init_params['input_size'],
